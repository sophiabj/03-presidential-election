...
def preprocess_op():
    ...
    return dsl.ContainerOp(
        ...
        image='gcr.io/kube01/kubeflow/presidentialelections/preprocess:latest',
        arguments=[],
        file_outputs={
...
def train_op(x_train, y_train):
    ...
    return dsl.ContainerOp(
        name='Train Model',
        image='gcr.io/kube01/kubeflow/presidentialelections/train:latest',
        arguments=[
            '--x_train', x_train,
            '--y_train', y_train
        ],
...
def test_op(x_test, y_test, model):
    ...
    return dsl.ContainerOp(
        name='Test Model',
        image='gcr.io/kube01/kubeflow/presidentialelections/test:latest',
        arguments=[
            '--x_test', x_test,
            '--y_test', y_test,
            '--model', model
...
client = kfp.Client()
client.create_run_from_pipeline_func(presidential_elections_pipeline, arguments={})
